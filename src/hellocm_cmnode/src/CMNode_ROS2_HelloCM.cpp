...
static struct {
    ...
    struct {
        ...
        struct {
         ...
         struct {
            int             Active;           /*!< Presence of active USonicRSI sensors */
            double*         pos;              /*!< Mounting position on vehicle frame */
            double*         rot;              /*!< Mounting rotation on vehicle frame*/
            int             UpdRate;
            int             nCycleOffset;
...
int CMNodeHelloCM::userVehicleControlCalc(const double& dt)
{
    ...
    if (CMNode.VC.use_vc == true) {
        ...
        else if (CMNode.VC.steer_ang > 0.1) {
          VehicleControl.Lights.IndR = 1;
        }
        else {
          VehicleControl.Lights.IndL = 0;
          VehicleControl.Lights.IndR = 0;
        }

        if (CMNode.VC.brake > 0) {
          VehicleControl.Lights.Brake = true;
        }
        else {
          VehicleControl.Lights.Brake = false;
        }
    }
    return 1;
}
...
void CMNodeHelloCM::LidarRSIFillMsg(sensor_msgs::msg::PointCloud2& msg) {
...
void CMNodeHelloCM::UltrasonicFillMsg(sensor_msgs::msg::Range& msg) {

  //const int Speed_of_Sound = 340; // in m/s

  if (CMNode.Sensor.USonicRSI.Active) {

    msg.header.stamp = rclcpp::Time(SimCore.Time);
    
    msg.field_of_view = 2.09; // 120 deg horizontal field of view in radians
    msg.min_range = 0.07;  // in m
    msg.max_range = 30;     // in m
    //msg.range = USonicRSI[0].ReceiverUSonic[0].nDetections; // integer number of detections

    float Time_of_Flight = 0;
    float dist_to_obj = 0;
    const int Speed_of_Sound = 340;
     
    for (int l = 0; l <= USonicRSI[0].ReceiverUSonic[0].nDetections; l++) {
      
      //int Time_of_Flight = 0;
      //int dist_to_obj = 0;
      Time_of_Flight = USonicRSI[0].ReceiverUSonic[0].DetPoints[l].TimeOfFlight;  // in s
      dist_to_obj = (Time_of_Flight * Speed_of_Sound) / 2; // in m 
      msg.range = dist_to_obj;
    } 
  }
}


// default IMUFillMsg with no orientation data, as the used InertialSensor does not provide it.

// void CMNodeHelloCM::IMUFillMsg(sensor_msgs::msg::Imu &msg)
// {
//   msg.header.stamp = rclcpp::Time(static_cast<int64_t>(1e9 * SimCore.Time), RCL_ROS_TIME);
//   msg.header.frame_id = "imu";

//   msg.linear_acceleration.x = InertialSensor[0].Acc_B[0];
//   msg.linear_acceleration.y = InertialSensor[0].Acc_B[1];
//   msg.linear_acceleration.z = InertialSensor[0].Acc_B[2];

//   msg.angular_velocity.x = InertialSensor[0].Omega_B[0];
...
// The ROS IMU topic currently publishes CarMaker inertial sensor quantities expressed in frame 0/global reference, using Acc_0 and Omega_0.
void CMNodeHelloCM::IMUFillMsg(sensor_msgs::msg::Imu &msg)
{
  msg.header.stamp = rclcpp::Time(static_cast<int64_t>(1e9 * SimCore.Time), RCL_ROS_TIME);
  msg.header.frame_id = "map"; // not imu

  msg.linear_acceleration.x = InertialSensor[0].Acc_0[0];
  msg.linear_acceleration.y = InertialSensor[0].Acc_0[1];
  msg.linear_acceleration.z = InertialSensor[0].Acc_0[2];

  msg.angular_velocity.x = InertialSensor[0].Omega_0[0];
  msg.angular_velocity.y = InertialSensor[0].Omega_0[1];
  msg.angular_velocity.z = InertialSensor[0].Omega_0[2];
...
void CMNodeHelloCM::GNSSFillMsg(sensor_msgs::msg::NavSatFix &msg)
{
  msg.header.stamp = rclcpp::Time(
      static_cast<int64_t>(1e9 * SimCore.Time),
      RCL_ROS_TIME);
  ...
  msg.latitude = GNavSensor.Receiver.UserPosLlhTsa_fromPsr[0] * rad2deg;
  msg.longitude = GNavSensor.Receiver.UserPosLlhTsa_fromPsr[1] * rad2deg;
  msg.altitude = GNavSensor.Receiver.UserPosLlhTsa_fromPsr[2];
...
void CMNodeHelloCM::GNSSLocalFillMsg(geometry_msgs::msg::PointStamped &msg)
{
  msg.header.stamp = rclcpp::Time(
      static_cast<int64_t>(1e9 * SimCore.Time),
      RCL_ROS_TIME);
  ...
  const double x = GNavSensor.Receiver.UserPosEcefTsa[0];
  const double y = GNavSensor.Receiver.UserPosEcefTsa[1];
  const double z = GNavSensor.Receiver.UserPosEcefTsa[2];
...
void CMNodeHelloCM::GroundTruthFillMsg(nav_msgs::msg::Odometry &msg)
{
  msg.header.stamp = rclcpp::Time(
      static_cast<int64_t>(1e9 * SimCore.Time),
      RCL_ROS_TIME);
  ...
  msg.child_frame_id = "vehicle_gt";

  tGroundTruthSensor *gt = GroundTruthSensor_GetByIndex(0);
  ...
  const tObservedPoint *p = nullptr;

  if (gt->mountedObject != nullptr &&
      gt->mountedObject->hasData &&
      gt->mountedObject->GTObjType == GTOT_Mount &&
      gt->mountedObject->data.mount != nullptr)
  {

    p = &(gt->mountedObject->data.mount->referencePoint);
  }

  if (p != nullptr)
  {
    msg.pose.pose.position.x = p->posFr0[0];
    msg.pose.pose.position.y = p->posFr0[1];
    msg.pose.pose.position.z = p->posFr0[2];

    msg.twist.twist.linear.x = p->velFr0[0];
    msg.twist.twist.linear.y = p->velFr0[1];
    msg.twist.twist.linear.z = p->velFr0[2];

    msg.twist.twist.angular.x = p->rotVelFr0[0];
    msg.twist.twist.angular.y = p->rotVelFr0[1];
    msg.twist.twist.angular.z = p->rotVelFr0[2];

    // Temporary: orientation not filled yet
    msg.pose.pose.orientation.w = 1.0;
  }
  else
  {
    // Fallback: sensor position only
    msg.pose.pose.position.x = gt->sensorPos0[0];
    msg.pose.pose.position.y = gt->sensorPos0[1];
    msg.pose.pose.position.z = gt->sensorPos0[2];
...
void CMNodeHelloCM::RadarFillMsg(geometry_msgs::msg::PoseArray &msg)
{
  msg.header.stamp = rclcpp::Time(
      static_cast<int64_t>(1e9 * SimCore.Time),
      RCL_ROS_TIME);
  ...
  tRadarSensor *radar = RadarSensor_GetByIndex(0);

  if (radar == nullptr || radar->ObjList == nullptr || radar->GlobalInf == nullptr)
  {
    return;
  }

  const int n_obj = radar->GlobalInf->nObj;

  msg.poses.clear();
  msg.poses.reserve(n_obj);

  for (int i = 0; i < n_obj; ++i)
  {
    const tOutQuants &obj = radar->ObjList[i];
    ...
    pose.position.x = obj.DistX;
    pose.position.y = obj.DistY;
    pose.position.z = obj.DistZ;
...
void CMNodeHelloCM::OdomFillMsg(nav_msgs::msg::Odometry& msg) {
...
void CMNodeHelloCM::RadarRSIFillMsg(sensor_msgs::msg::PointCloud2& msg) {
    ...
    for (int j = 0; j < RadarRSI[0].nDetections; j++) {
      ...
      points.z = RadarRSI[0].DetPoints[j].Coordinates[2];
        ...
        if (angle >= -30.0 && angle <= 30.0 && distance < 50) {
...
int CMNodeHelloCM::userTestrunStartAtBegin() {
  ...
  { /*  set up IMU publisher + job  */
    typedef CMJob::RosPublisher<sensor_msgs::msg::Imu> Imu_t;
    auto job = std::make_shared<Imu_t>(nhp_, "imu");     // check "Imu" filled if correct, after test run
    job->setCycleTime(100);                            // check cycle time, params file can be used or carmaker sensor c-code directly
    job->setCycleOffset(0);                            // check if required 
    job->registerCallback(&CMNodeHelloCM::IMUFillMsg, this);
    scheduler_.addJob(job);
  }

  //  ========= ODOMETRY =========
  { /* set up Odom publisher + job */
    typedef CMJob::RosPublisher<nav_msgs::msg::Odometry> Odom_t;
    auto job = std::make_shared<Odom_t>(nhp_, "odometry");
    job->setCycleTime(150);                            // check cycle time, params file can be used or carmaker sensor c-code directly
    job->setCycleOffset(0);                            // check if required 
    job->registerCallback(&CMNodeHelloCM::OdomFillMsg, this);
    scheduler_.addJob(job);

    Data_IF.TF_br = std::make_shared<tf2_ros::StaticTransformBroadcaster>(nhp_); // check this function and topic during run
  }

  //  ========= GLOBAL NAVIGATION SATELITE SYSTEM (gnss) =========
  {
    typedef CMJob::RosPublisher<sensor_msgs::msg::NavSatFix> Gnss_t;
    auto job = std::make_shared<Gnss_t>(nhp_, "gnss");
    job->setCycleTime(100);
    job->registerCallback(&CMNodeHelloCM::GNSSFillMsg, this);
    scheduler_.addJob(job);
  };

  {
    typedef CMJob::RosPublisher<geometry_msgs::msg::PointStamped> GnssLocal_t;
    auto job = std::make_shared<GnssLocal_t>(nhp_, "gnss_local");
    job->setCycleTime(100); // about 10 Hz, same as your fixed GNSS
    job->registerCallback(&CMNodeHelloCM::GNSSLocalFillMsg, this);
    scheduler_.addJob(job);
  };

  // ====== GROUND TRUTH ======
  {
    typedef CMJob::RosPublisher<nav_msgs::msg::Odometry> GroundTruth_t;
    auto job = std::make_shared<GroundTruth_t>(nhp_, "ground_truth");

    job->setCycleTime(100); // ~10 Hz, same as GNSS after your fix

    job->registerCallback(&CMNodeHelloCM::GroundTruthFillMsg, this);
    scheduler_.addJob(job);
  }

  // ====== RADAR =======
  {
    typedef CMJob::RosPublisher<geometry_msgs::msg::PoseArray> Radar_t;
    auto job = std::make_shared<Radar_t>(nhp_, "radar_objects"); 

    job->setCycleTime(100); // 10 Hz
    job->registerCallback(&CMNodeHelloCM::RadarFillMsg, this);
    scheduler_.addJob(job);
  };
  ...
  if (VehicleInfo_Err == 0) {
    ...
    { /* set up LidarRSI publisher + job */
      ...
      if (idxP != -1) {
        ...
        typedef CMJob::RosPublisher<sensor_msgs::msg::PointCloud2> Lidar;
        auto job = std::make_shared<Lidar>(nhp_, "pointcloud2_Lidar");
        job->setCycleTime(CMNode.Sensor.LidarRSI.UpdRate);
        job->setCycleOffset(CMNode.Sensor.LidarRSI.nCycleOffset);
        job->registerCallback(&CMNodeHelloCM::LidarRSIFillMsg, this);
        scheduler_.addJob(job);
      } else {
    ...
    { /* Find first RADAR RSI from vehicle infofile */
      idxP = -1;
      for (idxS = 0; idxS < CMNode.Misc.nSensors; idxS++) {
        sprintf(sbuf, "Sensor.%d.Ref.Param", idxS);
        ref = iGetIntOpt(Inf_Vehicle, sbuf, 0);
      ...
      if (idxP != -1) {
        sprintf(sbuf, "Sensor.%d.Active", idxS);
        CMNode.Sensor.RadarRSI.Active                   = iGetIntOpt(Inf_Vehicle, sbuf, 0);
        sprintf(sbuf, "Sensor.%d.pos", idxS);
        CMNode.Sensor.RadarRSI.pos                      = iGetFixedTableOpt2(Inf_Vehicle, sbuf, def3c, 3, 1);
        sprintf(sbuf, "Sensor.%d.rot", idxS);
        CMNode.Sensor.RadarRSI.rot                      = iGetFixedTableOpt2(Inf_Vehicle, sbuf, def3c, 3, 1);
        sprintf(sbuf, "Sensor.Param.%d.UpdRate", idxP);
        CMNode.Sensor.RadarRSI.UpdRate                  = iGetIntOpt(Inf_Vehicle, sbuf, 10);
        sprintf(sbuf, "Sensor.Param.%d.CycleOffset", idxP);
        CMNode.Sensor.RadarRSI.nCycleOffset             = iGetIntOpt(Inf_Vehicle, sbuf, 0);
      
      /* set up radar publisher + job */
        typedef CMJob::RosPublisher<sensor_msgs::msg::PointCloud2> PointCloud2_t;
        auto job = std::make_shared<PointCloud2_t>(nhp_, "PointCloud2_Radar");
        job->setCycleTime(CMNode.Sensor.RadarRSI.UpdRate);
        job->setCycleOffset(CMNode.Sensor.RadarRSI.nCycleOffset);
        job->registerCallback(&CMNodeHelloCM::RadarRSIFillMsg, this);
        scheduler_.addJob(job);
    }
    }
    ...
    { /*set up Ultrasonic RSI Publisher + job */

      /* Find each UltrasonicRSI in the vehicle info file*/ /* Currently done for only one USonic sensor */
      idxP = -1;
      for (idxS = 0; idxS < CMNode.Misc.nSensors; idxS++) {
        sprintf(sbuf, "Sensor.%d.Ref.Param", idxS);
        ref = iGetIntOpt(Inf_Vehicle, sbuf, 0);
        sprintf(sbuf, "Sensor.Param.%d.Type", ref);
        str = iGetStrOpt(Inf_Vehicle, sbuf, "");
        if (!strcmp(str, "USonicRSI")) {
          /* If the Ultrasonic sensor is found, get its index and extract its parameters */
          idxP = ref;
          break;
        }
      }

      /* Store USonicRSI sensor parameters */
        if (idxP != -1) {
          sprintf(sbuf, "Sensor.%d.Active", idxS);
          CMNode.Sensor.USonicRSI.Active                   = iGetIntOpt(Inf_Vehicle, sbuf, 0); 
          sprintf(sbuf, "Sensor.%d.pos", idxS);
          CMNode.Sensor.USonicRSI.pos                      = iGetFixedTableOpt2(Inf_Vehicle, sbuf, def3c, 3, 1);
          sprintf(sbuf, "Sensor.%d.rot", idxS);
          CMNode.Sensor.USonicRSI.rot                      = iGetFixedTableOpt2(Inf_Vehicle, sbuf, def3c, 3, 1);
          sprintf(sbuf, "Sensor.Param.%d.UpdRate", idxP);
          CMNode.Sensor.USonicRSI.UpdRate                  = iGetIntOpt(Inf_Vehicle, sbuf, 100);
          sprintf(sbuf, "Sensor.Param.%d.CycleOffset", idxP);
          CMNode.Sensor.USonicRSI.nCycleOffset             = iGetIntOpt(Inf_Vehicle, sbuf, 0);

          typedef CMJob::RosPublisher<sensor_msgs::msg::Range> Range_t;
          auto job = std::make_shared<Range_t>(nhp_, "Range_"+ std::to_string(idxS));
          job->setCycleTime(CMNode.Sensor.USonicRSI.UpdRate);
          job->setCycleOffset(CMNode.Sensor.USonicRSI.nCycleOffset);
          job->registerCallback(&CMNodeHelloCM::UltrasonicFillMsg, this);
          scheduler_.addJob(job);
        } else {
          CMNode.Sensor.USonicRSI.Active = 0;
        }
    } // set up UltrasonicRSI Publisher + job 


    { /* set up VehicleControl subscriber + job */
      std::string topic = "vehicle_control";
...
int CMNodeHelloCM::userTestrunEnd() {
  ...
  /*if (!CMNode.Sensor.CameraRSI.empty()) {     since rsds node is currently disabled 
    system("pkill -f carmaker_rsds_client_node > /dev/null 2>&1");
    CMNode.Sensor.CameraRSI.clear();
  }*/

  scheduler_.deleteJob("pointcloud2_Lidar");
  ...
  scheduler_.deleteJob("imu");
  scheduler_.deleteJob("odometry");
  scheduler_.deleteJob("gnss");
  scheduler_.deleteJob("gnss_local");
